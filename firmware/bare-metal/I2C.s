...
@ Function to receive data to I2C device with given address
    ...
    .thumb_func
        ...
        @ 1. Send MPU6050 Register address
            ...
            @ UDEBUG "WAIT FOR START BIT MPU6050"
            ...
            @ UDEBUG "START Condition complete - MPU6050"
            SYSTICK_SLEEP 10
            ...
            */
            ...
            @ Clear ADDR bit
            @ UDEBUG "Before ADDR bit clear"
            I2C2_ADDR_CLEAR_N_WAIT "i2c_mpu6050_receive_done"
            @ UDEBUG "ADDR bit cleared"   
            @------------------- CAN BE DEADLOCK------------ ONLY CHECKING SUCESS FLAG

            @ Send MPU6050's Register in I2C_DR
                @ Check for Data reg empty (TxE = 1)
                @ UDEBUG "TxE=1 check"
                I2C2_DR_EMPTY_TXE_WAIT "i2c_mpu6050_receive_done"
                @ UDEBUG "MPU's Address sent"
        ...
        I2C2_DR_EMPTY_TXE_WAIT "i2c_mpu6050_receive_done"  @ Wait for DR to empty (transfer complete)
        @ UDEBUG "MPU REG Address sent"
        ...
        @ 2. Reading from MPU6050
            ...
            @ UDEBUG "Start condition for Recive Generated"
...
.type i2c2_receive_RxNE_wait, %function
    .thumb_func
        i2c2_receive_RxNE_wait:
        @ RxNE bit in SR1 = 6
            PUSH {R0,R1}
            I2C2_RxNE_WAIT_LOOP:
            LDR R0, =(I2C2_BASE + I2C_SR1)

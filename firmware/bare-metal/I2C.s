...
.macro I2C2_DR_EMPTY_TXE_WAIT done_label:req
    PUSH {R0-R2, LR}
    BL i2c2_dr_empty_txe_wait
    LDR R1, =(0xFA14)
    CMP R0, R1 @ Check if ACK failed or not
    POP {R0-R2, LR}
    BEQ \done_label @ exit function if ACK failed
.endm

.macro I2C2_ADDR_CLEAR_N_WAIT done_label:req
    PUSH {R0-R2, LR}
    BL i2c2_addr_clear_wait
    LDR R1, =(0xFA14)
    CMP R0, R1 @ Check if ACK failed or not
    POP {R0-R2, LR}
    BEQ \done_label @ exit function if ACK failed
...
.macro I2C2_RxNE_WAIT
    @ RxNE bit in SR1 = 6
    PUSH {R0,R1}
    I2C2_RxNE_WAIT_LOOP_\@:
    LDR R0, =(I2C2_BASE + I2C_SR1)
    LDR R1, [R0]
    ANDS R1, R1, #(1<<6)
    BEQ I2C2_RxNE_WAIT_LOOP_\@
    POP {R0,R1}
.endm

.macro I2C2_DR_WRITE reg:req
    PUSH {R0-R4,LR}
    MOV R2, \reg
    @ Check for Data reg empty (TxE = 1)
    I2C2_DR_EMPTY_TXE_WAIT "i2c_mpu6050_send_done"

    @ Data bits = 7:0
    LDR R0, =(I2C2_BASE + I2C_DR)
    LDR R1, [R0]                @ I2C_DR content
    BIC R1, R1, #(0xFF)         @ Clear low 8 bits
    ...
    I2C2_DR_EMPTY_TXE_WAIT "i2c_mpu6050_send_done"  @ Wait for DR to empty (transfer complete)
    POP {R0-R4,LR}
.endm

.macro I2C2_ADDR_SEND reg:req rw:req
    @ rw = 0 -> write
    @ rw = 1 -> Read
    ...
    LDR R4, =(0xFFFFFF << 9 | 0x1) @ To clear unwanted bits and LSB bit
    LSL R2, R2, #(0x1)
    BIC R2, R4

    ORR R2, R2, #(\rw) @ To set Read / write mode
...
.section .text, "ax"
    ...
    .thumb_func
        ...
        @ ------------------------------------------------------- BUGGGGGG (if no wait it will not work)
            SYSTICK_SLEEP 10
            ...
            @ SELF TEST
                ...
                @ START bit = 8 (In Controller mode: 0: No Start generation / 1: Repeated start generation)
                    ...
                    @ UDEBUG "START BIT TEST IN I2C2 INIT"
...
@ Function to receive data to I2C device with given address
    ...
    .thumb_func
        i2c_mpu6050_receive:
            @ SEQUENCE: start controller transmit -> send MPU6050 register address -> switch controller recive -> recive MPU6050 register data
            ...
            LDR R5, =(0x68) @ MPU6050 slave address when ADO=0
        ...
        @ 1. Send MPU6050 Register address
            ...
            @ Send mpu's address byte in I2C_DR Register
            I2C2_ADDR_SEND R5 0x0
            @ Clear ADDR bit
            I2C2_ADDR_CLEAR_N_WAIT "i2c_mpu6050_receive_done"

            @ Send MPU6050's Register in I2C_DR
            I2C2_DR_WRITE R8

        @ 2. Reading from MPU6050

            @ Set the START bit in the I2C_CR1 register to generate a Start condition
            I2C2_SET_START_BIT

            @ Send mpu's address byte in I2C_DR Register
            I2C2_ADDR_SEND R5 0x1
...
.global i2c_mpu6050_send
    .type i2c_mpu6050_send, %function
    .thumb_func
        i2c_mpu6050_send:
        @ R6 = Data Input
        @ R7 = Data Size (in Bytes)
        ...
        @ 1. Send MPU6050 Register address
            ...
            @ Clear ADDR bit
            I2C2_ADDR_CLEAR_N_WAIT "i2c_mpu6050_send_done"
...
.type i2c2_dr_empty_txe_wait, %function
    .thumb_func
        i2c2_dr_empty_txe_wait:
            PUSH {R0-R2}
            @ TxE bit = 7 (0: Data register not empty / 1: Data register empty)

            I2C2_TxE_Wait:
                @ check for ACK fail (AF bit in SR1 = 10) (0: No acknowledge failure / 1: Acknowledge failure)
                LDR R0, =(I2C2_BASE + I2C_SR1)
                LDR R2, [R0]
                ANDS R2, R2, #(1<<10)
                BNE I2C2_DR_ACK_FAIL
            ...
            I2C2_DR_ACK_FAIL:
                USART_SEND "[ERROR]: I2C2 DR ACK FAILED \r\n"
                @ Send STOP condition
                I2C2_SET_STOP_BIT
...
.type i2c2_addr_clear_wait, %function
    .thumb_func
        i2c2_addr_clear_wait:
            PUSH {R0-R2}
            ...
            @ Read I2C_SR1 followed by I2C_SR2 to clear ADDR bit

            LDR R0, =(I2C2_BASE + I2C_SR1)
            LDR R1, [R0]
            LDR R0, =(I2C2_BASE + I2C_SR2)  
            LDR R1, [R0]

            @ Read I2C_SR1 to check ADDR
                @ ADDR bit = 1
            LDR R0, =(I2C2_BASE + I2C_SR1)
            LDR R1, [R0]
            ANDS R1, R1, #(0x2)
            BNE I2C2_ADDR_CLEAR_WAIT   @ Read SR1 and SR2 again if ADDR bit is not cleared

            @ UDEBUG "ADDR BIT IS CLEARED"

            POP {R0-R2}
            BX LR

            I2C2_ADDR_ACK_FAIL:
                USART_SEND "[ERROR]: I2C2 ADDR ACK FAILED \r\n"
                @ Send STOP condition
                I2C2_SET_STOP_BIT
